export default function BetCard({ bet }) {
    ...
    const isWin = normalizedStatus === 'win' || normalizedStatus === 'won';
    const isLoss = normalizedStatus === 'loss' || normalizedStatus === 'lost';
    const isVoid = ['push', 'refund', 'void'].includes(normalizedStatus);
    ...
    return (
        <div className="flex gap-4 items-stretch mb-3 group">
            ...
            <div className={`hidden md:flex flex-col items-end justify-center min-w-[60px] text-right transition-colors duration-300 ${isWin ? 'text-emerald-400' : isLoss ? 'text-rose-400' : 'text-white/80'}`}>
            ...
            <div className={`flex-1 grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 rounded-xl border transition-all duration-300 ${style.bg} ${style.bg.replace('/10', '/15')} ${style.text.replace('text-', 'border-').replace('400', '500/30')} hover:brightness-110 relative overflow-hidden`}>
                ...
                <div className={`absolute left-0 top-0 bottom-0 w-1 ${isWin ? 'bg-emerald-500' : isLoss ? 'bg-rose-500' : 'bg-yellow-500/50'}`} />
                ...
                <div className="md:col-span-4 flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {bet.score && (
                                <span className={`px-1.5 py-0.5 rounded text-[10px] md:text-xs font-mono font-bold border ${isWin ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : isLoss ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>
                                    {bet.score}
                                </span>
                            )}
                        </div>
                ...
                <div className="md:col-span-3 flex items-center justify-end gap-4">
                    ...
                    <div className={`px-2 py-2 md:px-4 rounded-lg border font-bold uppercase tracking-[0.1em] text-[10px] min-w-[80px] md:min-w-[90px] text-center shadow-sm transition-all duration-300
                        ${isWin ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30 shadow-emerald-500/10' :
                            isLoss ? 'bg-rose-500/20 text-rose-400 border-rose-500/30 shadow-rose-500/10' :
                                isVoid ? 'bg-gray-500/20 text-gray-400 border-gray-500/30' :
